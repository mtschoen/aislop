...
export const isAutoGenerated = (filePath: string): boolean => {
	...
	// Normalize to forward slashes so the anchored patterns match Windows paths too.
	const lower = filePath.toLowerCase().split(path.sep).join("/");
	if (
