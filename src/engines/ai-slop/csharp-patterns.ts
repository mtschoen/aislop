...
// Debug/Trace output left in shipped code. Non-production paths (tests, scripts,
// examples) are skipped - debug output there is expected. `Console.*` is handled
// in Phase B (it needs project-output-type context). Mirrors the JS
// console-leftover heuristic in dead-patterns.ts.
const flagConsoleLeftover = (lines: string[], relPath: string, out: Diagnostic[]): void => {
...
export const detectCSharpPatterns = async (context: EngineContext): Promise<Diagnostic[]> => {
	...
	for (const filePath of files) {
		...
		flagConsoleLeftover(lines, relPath, diagnostics);
	}
