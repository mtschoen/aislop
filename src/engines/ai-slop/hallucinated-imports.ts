...
import {
	checkJsImport,
	collectDeclaredModuleMatchers,
	extractJsImports,
} from "./hallucinated-imports-js.js";
import {
	jsDepsForFile,
	loadManifest,
	pythonDepsForFile,
	pythonImportRootForFile,
	readJson,
...
const isLocalPythonImport = (spec: string, filePath: string, rootDirectory: string): boolean => {
	...
	while (searchDirectory === root || searchDirectory.startsWith(`${root}${path.sep}`)) {
		...
		if (
			isSafePythonModuleEntry(`${candidate}.py`, root, "file") ||
			isSafePythonModuleEntry(candidate, root, "directory")
		) {
			return true;
		}
...
export const detectHallucinatedImports = async (context: EngineContext): Promise<Diagnostic[]> => {
	...
	const declaredModuleMatchers = manifest.hasJsManifest
		? collectDeclaredModuleMatchers(projectFiles, context.rootDirectory)
		: [];
	...
	for (const filePath of files) {
		...
		const pyImportRoot = isPy
			? pythonImportRootForFile(manifest, filePath, context.rootDirectory)
