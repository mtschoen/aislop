...
import { buildDocstringRanges } from "../../utils/python-docstrings.js";
import { maskComments } from "../../utils/source-masker.js";
import { getSourceFiles, isAutoGenerated } from "../../utils/source-files.js";
import { isTestFile } from "../../utils/source-file-policy.js";
import type { Diagnostic, EngineContext } from "../types.js";
import { isNonProductionPath } from "./non-production-paths.js";
import { detectRepeatedMagicLiteralsInFile } from "./repeated-magic-literals.js";
...
const DEFAULT_REPEATED_LITERAL_THRESHOLD = 3;
...
const scanFileForConfigLiterals = (
	...
	// languages have no equivalent "documentation, not code" string convention here.
	const docstringLines = ext === ".py" ? buildDocstringRanges(lines) : null;
	return lines.flatMap((line, index) =>
		scanLineForConfigLiterals(
			line,
			relativePath,
			ext,
			index + 1,
...
export const detectHardcodedConfigLiterals = async (
	...
	for (const filePath of getSourceFiles(context)) {
		...
		const commentMaskedContent = maskComments(content, ext);
		diagnostics.push(...scanFileForConfigLiterals(commentMaskedContent, relativePath, ext));
		if (!isTestFile(relativePath)) {
			diagnostics.push(
				...detectRepeatedMagicLiteralsInFile(
					content,
					relativePath,
					ext,
					context.config.quality.repeatedLiteralThreshold ?? DEFAULT_REPEATED_LITERAL_THRESHOLD,
				),
			);
		}
	}
