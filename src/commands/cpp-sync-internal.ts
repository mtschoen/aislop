...
/**
 * Finds the index of the innermost opening brace `{` that encloses `fromIndex`
 * in `contents` (where comments, strings, and preprocessors have been blanked).
 * Returns -1 if `fromIndex` is at file / global scope (brace depth 0).
 */
const findInnermostEnclosingBrace = (contents: string, fromIndex: number): number => {
	let depth = 0;
...
/**
 * Checks whether `nameStart` sits directly inside an enclosing scope that is
 * permitted to contain free function definitions (file scope, namespaces, and
 * extern linkage blocks). Structs, classes, unions, enums, function bodies, and
 * control flow blocks cannot contain free function definitions.
 */
const isAllowedEnclosingScope = (contents: string, nameStart: number): boolean => {
	const openBraceIndex = findInnermostEnclosingBrace(contents, nameStart);
	if (openBraceIndex === -1) {
		// File / global scope is always allowed.
		return true;
	}
	// Find the start of the construct that opened this brace.
	let start = openBraceIndex - 1;
	while (start >= 0 && !"{};".includes(contents[start] ?? "")) start -= 1;
	const header = contents.slice(start + 1, openBraceIndex).trim();
	const words = header.match(/[A-Za-z_]\w*/g) ?? [];
	// Structs, classes, unions, and enums define member functions/types, not free helpers.
	if (
		words.some(
			(word) => word === "struct" || word === "class" || word === "union" || word === "enum",
		)
	) {
		return false;
	}
	// Namespaces and extern linkage blocks may contain free function definitions.
	if (words.some((word) => word === "namespace" || word === "extern")) {
		return true;
	}
	// Any other construct (e.g. function bodies, control flow) cannot contain free functions.
	return false;
};
 ...
 * where a return type cannot legally appear (after `=`, a lambda capture, a
 * `.`/`->` member access, or a `::` scope qualifier), which is how lambdas, call
 * expressions, and qualified member definitions are rejected.
 */
const declarationStart = (contents: string, nameStart: number): number | null => {
	...
	// An assignment, member access (./->), or scope qualification (::) to the
	// left means an initializer, lambda, member call, or qualified member
	// definition, not a free function definition whose return type we can lift
	// into a declaration.
...
const extractDefinitions = (
	...
	while ((match = namePattern.exec(scanContents)) !== null) {
		...
		if (!isAllowedEnclosingScope(scanContents, nameStart)) continue;
...
const referencesName = (contents: string, name: string): boolean =>
	new RegExp(`(?<!(?:\\.|->|\\w\\s*::)\\s*)\\b${escapeRegExp(name)}\\s*\\(`).test(contents);
