...
describe("detectTrivialComments", () => {
	...
	it("does not flag a handful of terse comments spread across a long hand-written file", async () => {
		// Mirrors express lib/application.js: ~3 short verb-led comments in ~600 lines.
		// Sparse terse comments are ordinary human style, not machine narration.
		const body: string[] = [];
		for (let i = 0; i < 200; i++) {
			body.push(`const value${i} = compute${i}(input);`);
			if (i === 40) body.push("// setup locals");
			if (i === 41) body.push("this.locals = Object.create(null);");
			if (i === 90) body.push("// Setup getting to lazily add base router");
			if (i === 91) body.push("Object.defineProperty(this, 'router', {});");
			if (i === 150) body.push("// copy arguments");
			if (i === 151) body.push("var args = new Array(arguments.length);");
		}
		const filePath = writeFile("application.js", body.join("\n"));
		const diagnostics = await detectTrivialComments(makeContext([filePath]));
		expect(diagnostics).toHaveLength(0);
	});

	it("still flags the same terse comments when they saturate a file", async () => {
		// Same comment text, but narrated every other line: the machine-generated shape.
		const body: string[] = [];
		for (let i = 0; i < 12; i++) {
			body.push("// setup locals");
			body.push(`this.locals${i} = Object.create(null);`);
		}
		const filePath = writeFile("generated.js", body.join("\n"));
		const diagnostics = await detectTrivialComments(makeContext([filePath]));
