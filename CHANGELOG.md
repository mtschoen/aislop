...
- **C/C++ support.** aislop now scores C and C++ repositories as a first-class language target (`.c`, `.cpp`, `.h`, `.hpp`, `.cc`, `.cxx`). Detection and scoring treat C/C++ files the same as other supported languages, so mixed-language repos that include C/C++ are scored and reported correctly.
  ...
  - **clang-tidy lint** (`clang-tidy/*` rules) runs only when a `compile_commands.json` is present in the project root (generated by CMake with `-DCMAKE_EXPORT_COMPILE_COMMANDS=ON` or by Bear). clang-tidy is also a system tool, not bundled. Findings from cppcheck and clang-tidy are merged and de-duplicated by `(filePath, line, bare-rule-id)`.
