...
- **Native Codex hooks.** `aislop hook install --codex` now writes documented `hooks.json` PostToolUse configuration in addition to fenced `AGENTS.md` rules. Global callbacks no-op unless the current project contains `.aislop/config.yml`, preserving a quiet systemwide install for shared Codex workers. `--quality-gate` adds an optional Stop callback that blocks score regressions; install and uninstall preserve unrelated hooks and symlinked instruction files.
- **C/C++ support.** aislop now scores C and C++ repositories as a first-class language target (`.c`, `.cpp`, `.h`, `.hpp`, `.cc`, `.cxx`). Detection and scoring treat C/C++ files the same as other supported languages, so mixed-language repos that include C/C++ are scored and reported correctly.
  - **cppcheck lint** (`cppcheck/*` rules) runs on any C/C++ checkout. cppcheck is a system tool aislop shells out to - it is not bundled.
  - **clang-tidy lint** (`clang-tidy/*` rules) runs only when a `compile_commands.json` is present (probed at the project root and one level under `build/`/`out/`/`cmake-build*`, e.g. `build/lint/`; generated by CMake with `-DCMAKE_EXPORT_COMPILE_COMMANDS=ON` or by Bear). clang-tidy is also a system tool, not bundled. It is restricted to the translation units the compile database describes, so a platform-specific source (e.g. a POSIX `.cpp` on a Windows database) is not analyzed with guessed flags. Findings from cppcheck and clang-tidy are merged and de-duplicated by `(filePath, line, canonical-rule-id)`.
  - **jb inspectcode / ReSharper C++** (`jb/Cpp*` rules), OFF by default (`lint.cpp.jb`). The same JetBrains CLI used for C# also inspects C++ projects in a loadable solution (requires Windows + MSVC). When enabled, aislop runs a single inspectcode pass over the union of the C# and C++ project scopes and partitions results by language; jb's clang-tidy-backed inspections are de-duplicated against standalone clang-tidy via a canonical rule id. Configurable via `lint.cpp.jb` / `jbSeverityFloor` / `jbExcludeTypes` / `jbProjects`.
  - cppcheck now passes `--language=c++` for C++ trees (so ambiguous `.h` headers are not mis-parsed as C) and suppresses its parse-context self-diagnostics (`syntaxError`, `unknownMacro`, `internalAstError`, `internalError`) that fire only when a build macro/include it cannot see is missing.
