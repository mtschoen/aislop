...
| C/C++ | cppcheck + clang-tidy (clang-tidy requires `compile_commands.json`; both are system tools, not bundled) |
...
### C/C++ linting: cppcheck and clang-tidy

The C/C++ lint pass shells out to two system tools (neither is bundled):
...
Findings from both tools are merged and de-duplicated by `(filePath, line, bare-rule-id)`.
